# BLE Mesh Light Smart Demo application

## Overview
This Light Smart demo application shows a simple implementation of a Light Controller. The app is based on the snip/mesh/mesh\_light\_lc sample which implements BLE Mesh Light LC (Lightness Control) Server model. As a Light LC device, the Light Smart application allows two types of the controls depending on the Light LC Mode state. If Light LC Mode is on, the application is controlled by the occupancy sensor input and can be also controlled by the Light LC On/Off or  Generic On/Off messages sent to the Light LC element of the device. If Light LC Mode is off, the Light Smart
